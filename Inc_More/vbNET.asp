...
function getAspToVbNetContent(content,language, msgStr)
    ...
    dim obj : set obj = new aspToLanguageClass
		...
		if language="vb.net" then
			c="|bin2Str|binVal|binVal2|bin2Int|arrayToBin|midBin|"			'对二进制操作，ASP与VB.Net不一样，晕，等以后看看，
...
class aspToLanguageClass
    ...
    function aspToLanguage(byVal content)
        ...
        for nI = 0 to uBound(splStrRow)
            ...
            if left(pHPTrim(lCase(s)), 1) = "." and ( (instr(s,"""")>0 or instr(s,",")>0) and instr(s,"(")=false) then
				'call echo("s",s)
                s = handleFullRowAspCode(s)  				'处理这种 .open "GET", httpurl, false '测试页面是否存在
			'隐藏代码
	...
	function handleLanguage(varName,sType) 
		...
		if sType="变量" then
			varName=handleToSystemVar(varName)			'让系统变量大小写完整，要不然在vb.net里会报错
		elseif sType="输入" then
			handleLanguage = """" & replace(varName,"“",""" & Chr(-24144) & """) & """"			'处理” 因为在VB.net里不能用
			exit function
		end if
        ...
        elseIf action = "function" or action = "sub" then
			...
			elseIf sType = "变量" then
				...
				'对函数参数处理，因为在vb.net里，默认变量是同步为byval  而在asp里则相反byref
    ...
    private sub class_Initialize()
		...
		isServerMapPath=false										'是否为系统路径，默认为假
    end sub 
	...
	function handleToSystemVar(varName)        
		...
		c = c & ",Is_object_,Create_object_,Get_object_,InStrB,AscB,MidB,LenB,ChrB,AscW,ChrW, " '自己定义的
	...
	'处理前   .open "GET", httpurl, false 															 '测试页面是否存在
	'处理后   Call .Open("GET", httpurl, False)                                                      '测试页面是否存在
	'处理完整一行ASP代码 20161004
	function handleFullRowAspCode(s) 
		...
		s=phpRTrim(s)
		startStr=mid(s,1,instr(s,".")-1)				'.点前面空格
		endStr=mid(s,instr(s,"."))						 '包括.点后面内容
		endStrLeft=mid(endStr,1,instr(endStr," ")-1) & "("		'第一个空格前 追加(
		endStrRight=mid(endStr,instr(endStr," ")+1)				'第一个空格后
		if instr(endStrRight,"'")>0 then						'有注释'号
